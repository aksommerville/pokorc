...
const char *png_validate(const void *src,int srcc) {
  if (!src) return "null input";
  if (srcc<1) return "empty input";
  if ((srcc<8)||memcmp(src,"\x89PNG\r\n\x1a\n",8)) return "signature mismatch";
  const uint8_t *SRC=src;
  int srcp=8,have_IHDR=0,have_IDAT=0,have_IEND=0;
  while (1) {
  
    // Generic chunk check.
    if (srcp>srcc-8) return "unexpected end of file";
    int len=(SRC[srcp]<<24)|(SRC[srcp+1]<<16)|(SRC[srcp+2]<<8)|SRC[srcp+3]; srcp+=4;
    int id=(SRC[srcp]<<24)|(SRC[srcp+1]<<16)|(SRC[srcp+2]<<8)|SRC[srcp+3]; srcp+=4;
    if (len<0) return "invalid chunk length";
    if (srcp>srcc-len) return "unexpected end of file";
    const uint8_t *v=SRC+srcp;
    srcp+=len;
    if (srcp>srcc-4) return "unexpected end of file";
    srcp+=4; // we could check the CRC but who cares
    
    switch (id) {
      case PNG_ID('I','H','D','R'): {
          if (len<13) return "invalid IHDR size";
          if (have_IHDR) return "multiple IHDR";
          have_IHDR=1;
        } break;
      case PNG_ID('I','D','A','T'): {
          have_IDAT=1;
      ...
      case PNG_ID('I','E','N','D'): {
          if (!have_IHDR) return "IHDR missing";
          if (!have_IDAT) return "IDAT missing";
          return 0;
        }
    }
  }
